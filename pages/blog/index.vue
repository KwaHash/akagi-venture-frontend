    ...
    <div :class="s.inner">
      ...
      <ul :class="s.list">
        <li
          ...
          <nuxt-link v-else :to="`/blog/${post.slug}/`" :class="s.listLink">
            <span :class="s.listDate">{{ post.publish_date }}</span>
            <span :class="s.listBrand">{{ post.site }}</span>
            <h3 :class="s.listTitle">{{ post.title }}</h3>
          </nuxt-link>
    ...
    <div v-if="maxPage" :class="s.pagenate">
      <ul :class="s.pagenate_list">
        <li v-if="!pages.includes(1)">
          <nuxt-link :class="s.list" to="/blog?page=1">
            1
          </nuxt-link>
        ...
        <li
          v-for="p in pages"
          :key="p">
          <nuxt-link :class="p === Number($route.query.page) ? s.list_now : s.list" :to="`/blog?page=${p}`">{{ p }}</nuxt-link>
        </li>
        <li v-if="!pages.includes(maxPage)">
          <span v-if="!pages.includes(maxPage - 1)" :class="s.center">・・・</span>
          <nuxt-link :class="s.list" :to="`/blog?page=${maxPage}`">
            {{ maxPage }}
          </nuxt-link>
        </li>
      </ul>
