    ...
    <div :class="s.inner">
      ...
      <ul :class="s.list">
        <li
          ...
          <nuxt-link v-else :to="`${ env.envName === 'local' ? '/bacon' : '' }/blog/${post.slug}/`" :class="s.listLink">
            <span :class="s.listDate">{{ post.publish_date }}</span>
            <h3 :class="s.listTitle">{{ post.title }}</h3>
          </nuxt-link>
        </li>
    ...
    <div v-if="maxPage" :class="s.pagenate">
      <ul :class="s.pagenate_list">
        <li v-if="!pages.includes(1)">
          <nuxt-link
            :class="s.list"
            :to="`${ env.envName === 'local' ? '/bacon' : '' }/blog?page=1`">
            1
          </nuxt-link>
        ...
        <li
          ...
          <nuxt-link
            ...
            :to="`${ env.envName === 'local' ? '/bacon' : '' }/blog?page=${p}`">
            {{ p }}
          </nuxt-link>
        </li>
        <li v-if="!pages.includes(maxPage)">
          ...
          <nuxt-link
            :class="s.list"
            :to="`${ env.envName === 'local' ? '/bacon' : '' }/blog?page=${maxPage}`">
            {{ maxPage }}
          </nuxt-link>
        </li>
      </ul>
...
.list {
  margin-top: 70px;
  border-bottom: 1px solid #001ca5;
}

.listItem {
  border-top: 1px solid #001ca5;
}
