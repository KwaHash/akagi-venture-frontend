<template>
  <div id="anchor-camp" :class="s.container">
    <ThinContainer :class="s.inner">
      <div :class="s.contentsContainer">
        <h2 :class="s.heading">手ぶら<br>デイキャンプ</h2>
        <div :class="s.contents">
          <div :class="[s.contentsItem, s.isActive]" >
            <div :class="s.contentInner">
              <div :class="s.title">ご利用にあたって</div>
              <div :class="s.smallThumbContainer">
                <img :src="`/images/brand/bacon/camp/0.jpg`" :class="s.smallThumb" alt="手ぶらデイキャンプ">
              </div>
              <div :class="s.text">
                <div :class="s.table">
                  <div :class="s.tableRow">
                    <div :class="s.headCell">料金</div>
                    <div :class="s.dataCell">¥22,000／サイト</div>
                  </div>
                  <div :class="s.tableRow">
                    <div :class="s.headCell">お時間</div>
                    <div :class="s.dataCell">フルタイム / 10:00~17:00</div>
                  </div>
                  <div :class="s.tableRow">
                    <div :class="s.headCell">利用人数</div>
                    <div :class="s.dataCell">2〜6名(子供人数要相談)</div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div :class="s.sliderContainer">
        <div :class="s.slider">
          <div :class="[s.sliderItem, s.isActive]" >
            <img :src="`/images/brand/bacon/camp/0.jpg`" alt="">
          </div>
        </div>
      </div>
    </ThinContainer>
...
.table {
  // レスポンシブの必要があるときに面倒そうなのでtableタグではやらない
  display: block;
}
...
.navIcon {
  ...
  .nav:active &.prev {
    transform: translate3d(-40%, 0, 0) scale3d(1.3, 1, 1);
  }
